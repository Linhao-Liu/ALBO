...
df = df[df['Crystallitesize(nm)'] <= 75]
print(df.shape)
...
# Step 3: Use PCA to reduce high-dimensional features to 2D for visualization
...
# Step 4: Plot cluster distribution in 2D space
...
# Model names and their corresponding prediction results
...
for i, (name, preds) in enumerate(zip(model_names, model_preds)):
    ...
    # Concatenate all test set actual and predicted values (10-fold CV results)
    ...
    # Approximate training predictions using test models (since training preds are not saved)
    y_train_all = np.concatenate([np.array(y_train[k]) for k in range(10)])
    y_train_pred = []
    for k in range(10):
        # Approximation: use test models to estimate train predictions
        y_train_pred.extend([np.mean([model_preds[i][j][0][m] for j in range(10) if m < len(y_test[j])])
                             for m in range(len(y_train[k]))])
    y_train_pred = np.array(y_train_pred)

    # Scatter plot: actual vs predicted values for both train and test sets
...
for name, pred_var, mae_var in model_specs:
    if pred_var in globals() and mae_var in globals():
        ...
        # Collect predictive uncertainties (concatenate per-fold std arrays)
        stds_all = []
        for k in range(len(preds)):              # use len(preds) to avoid out-of-range errors
            stds_k = np.asarray(preds[k][1]).ravel()   # extract index 1 = std (uncertainty)
        ...
        print(f"[skip] {pred_var} or {mae_var}， skipping this model")

# Sanity check
assert len(model_names) > 0, "No valid model data found. Please check variable names."

# Sort models by mean MAE (ascending order) for better visualization
...
# Recompute uncertainty list with a lower clipping threshold
uncertainty_list = []
for preds in [xgb_y_pred, svr_y_pred, gbr_y_pred, dtr_y_pred, abr_y_pred,
    ...
    for k in range(len(preds)):
        ...
        # Set a numerical lower bound to avoid underflow
        stds_k = np.clip(stds_k, 1e-3, None)
        stds.append(stds_k)
    uncertainty_list.append(np.concatenate(stds))

# Boxplot of predictive uncertainty (log scale)
...
# ===== 2) Per-fold MAE boxplot (more interpretable than histograms) =====
plt.figure(figsize=(10,5))
sns.boxplot(data=mae_list, whis=[5,95], showfliers=False)
meds = [np.median(x) for x in mae_list]
plt.scatter(range(len(meds)), meds, zorder=3)
plt.xticks(range(len(model_names)), model_names)
...
#  1) Construct 60 columns (first 59 are features, last one is target y)
_prop_labels = [
...
feat_names.extend(process_names)

assert len(feat_names) == df_mean.shape[1], \
    f"Number of feature names({len(feat_names)})does not match df_mean columns ({df_mean.shape[1]}). Please verify concatenation order."

# Assign human-readable column names to df_mean (non-destructive copy)
df_mean_named = df_mean.copy()
df_mean_named.columns = feat_names

# 2) Define features (X) and target (y)
# The model uses 59 features; retrieve y from the original df to prevent data leakage
assert 'Crystallitesize(nm)' in df.columns, "Target column 'Crystallitesize(nm)' not found in the original dataframe."
...
shap.summary_plot(shap_arr, X_test, plot_type='dot', show=False)
...
# Export numerical SHAP importance table (for paper appendix)
...
# Print Top-10 most influential features for inspection
topN = 10
